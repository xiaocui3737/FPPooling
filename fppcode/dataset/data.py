...
class HSSMS(LSSNS):
    ...
    def raw_dir(self) -> str:
        return osp.join(self.root, self.name.split('-')[0], 'raw')

    @property
    def processed_dir(self) -> str:
        return osp.join(self.root, self.name.split('-')[0], self.name)

    @property
    def raw_file_names(self) -> str:
        print(self.name.split('-'))
        dt_name = self.name.split('-')[0]
    ...
    def download(self):
        url = self.url.format(self.names[self.name.split('-')[0]][1])
        path = download_url(url, self.raw_dir)
        if self.names[self.name.split('-')[0]][1][-2:] == 'gz':
            extract_gz(path, self.raw_dir)
            os.unlink(path)
...
class MoleculeNet2(InMemoryDataset):
    ...
    **STATS:**
    ...
    def __init__(
        ...
        self.name = name.lower()
        assert self.name.split('-')[0] in self.names.keys()
        super().__init__(root, transform, pre_transform, pre_filter)
        self.data, self.slices = torch.load(self.processed_paths[0])
        ...
        if self.name.split('_')[0] == 'muv':        
            ...
            for k, line in enumerate(dataset):
                ...
                try:
                    ...
                    smiles = line[self.names[self.name.split('-')[0]][3]]
                    mol = Chem.MolFromSmiles(smiles)
                    if mol == None:
                        continue
                    ys = line[self.names[self.name.split('-')[0]][4]]
                    ys = ys if isinstance(ys, list) else [ys]
